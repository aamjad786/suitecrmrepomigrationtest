...
// AWS access info
if (!defined('awsAccessKey')) define('awsAccessKey', getenv('SCRM_AWS_ACCESS_KEY'));
if (!defined('awsSecretKey')) define('awsSecretKey', getenv('SCRM_AWS_ACCESS_SECRET'));

function uploadFile ($uploadFile='audio.mp3'){ // File to upload, we'll use the S3 class since it exists
// echo $uploadFile;die();
$folder = date('Ym');
$file = 'ozonetel/'.$folder.'/'.basename($uploadFile);
// die($file);
rename('audio.mp3',$file);
// die($file);
$bucketName = 'ng-ivrs-recordings';//uniqid('s3test'); // Temporary bucket
...
//	echo "Created bucket {$bucketName}".PHP_EOL;
	...
	if ($s3->putObjectFile($uploadFile, $bucketName, $file, S3::ACL_PUBLIC_READ)) {
