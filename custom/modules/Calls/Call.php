	...
	*/
	...
	function create_list_query($order_by, $where, $show_deleted=0)
	{
        ...
        function create_export_query($order_by, $where, $relate_link_join='')
        {
			...
			// var_dump($contact_required);die();
            if($contact_required)
            {
                    $query = "SELECT calls.*,calls_cstm.*, contacts.first_name, contacts.last_name, users.user_name as assigned_user_name ";
                    $query .= $custom_join['select'];
                    $query .= " FROM calls_cstm,users,contacts, calls, calls_contacts ";
            ...
            {
                    $query = 'SELECT calls.*,calls_cstm.*, users.user_name as assigned_user_name ';
                    $query .= $custom_join['select'];
                    $query .= ' FROM calls,calls_cstm ';
                    $where_auto = "calls.deleted=0";
            }
			$query = "select
			calls.direction as 'Direction',
			calls_cstm.app_id_c as 'App Id',
            calls_cstm.calls_final_status_c as 'Final Status',
			calls_cstm.calls_type_c as 'Calls type',
			calls_cstm.date_requested_c as 'Requested Date',
            calls_cstm.account_number_c as 'Account Number',
            calls_cstm.ifsc_code_c as 'IFSC code',
            calls_cstm.refund_amount_c as 'Refund Amount',
            calls_cstm.contact_number_c as 'Contact Number',
            calls_cstm.repayment_mode_c as 'Repayment Mode',
            calls_cstm.funded_date_c as 'Funded Date',
            calls_cstm.establishment_name_c as 'Establishment',
            calls_cstm.email_id_c as 'Email',
            calls_cstm.loan_status_c as 'Loan Status',
            calls_cstm.branch_c as 'Branch',
            Concat(users.first_name,' ',users.last_name) as 'Closed by',
            cases.case_number as 'Case Number',
            cases_cstm.case_category_c as 'Case category',
            cases_cstm.case_subcategory_c as 'Case subcategory',
            group_concat(calls_reschedule.date_entered) as 'Attempt Dates',
            group_concat(calls_reschedule.reason) as 'Attempt Reasons',
            calls.name as 'Subject',
            calls.description as 'Call Description',
            calls.date_start as 'Call Date',
			calls_cstm.calls_action_c as 'Call Action',
            calls.id from calls 

			left join users on calls.parent_id = users.id
			left join calls_cstm on calls.parent_id = calls_cstm.id_c
            left join cases_cstm on calls.parent_id = cases_cstm.id_c  
            left join cases on calls.parent_id = cases.id
            left join calls_reschedule on calls.id = calls_reschedule.call_id ";
            $where_auto = "calls.deleted=0";
