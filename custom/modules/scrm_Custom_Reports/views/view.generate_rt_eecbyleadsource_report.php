...
class scrm_Custom_ReportsViewgenerate_rt_eecbyleadsource_report extends SugarView {
	...
	function getMatrixData($post)
	{
		...
		while($row = $db->fetchByAssoc($result))
		{
			//echo "here";
			 ...
			 // continue;
			 //die();
			/*[id] => 1442a584-90f3-78de-5e52-57b1e1c266be [name] => tes2 
			[date_entered] => 2016-08-15 15:35:12 
			[date_modified] => 2016-08-23 05:59:00 
			[modified_user_id] => 1 [created_by] => 1 
			[description] => Want funds to add stocks? Get Business loan from NeoGrowth. Give Missed call 09222272881 (T&C Apply) 
			[deleted] => 0 [assigned_user_id] => 
			[sms_template_id] => 47c00913-a9c3-61b2-a475-57837fd855fc 
			[target_list_id] => 105c8d47-0555-2be6-7641-57ac2c10ae28 
			[scheduled_time] => 2016-08-15 21:05:00 [entered_by] => 
			[total_count] => 0 [absent_subscriber_count] => 0 
			[blacklist_count] => 0 [duplicate_msg_count] => 4 
			[delivered_count] => 4 [expired_count] => 0 [invalid_subscriber_count] => 0 
			[inbox_full_count] => 0 [ndnc_count] => 0 [queued_count] => 2 
			[submitted_to_nw_count] => 0 [undelivered_count] => 0 [is_processed] => 1 
			[is_personalized] => 0 [save_only] => 1*/
			...
			$rowid = $row['id'];
				  ...
				  //$prefix ="91";
			   //$query_no_message_response = "SELECT count(n.id) as message_response from net_missed_calls n, sms_sms s where concat($prefix,n.name) =s.name and n.deleted=0 and s.deleted=0 and $from_date $to_date  and n.receiving_number='9222272881' Group By s.date_entered";
			  // $missed_call_query = "SELECT count(n.id)  as message_response from net_missed_calls n join sms_sms s where concat('91',n.name) =s.name and n.deleted=0 and s.deleted=0  and n.receiving_number='9222272881' and s.description like '%$msg_substr%' and date(n.date_entered) >= date('$rowdate')";
			  //~ $query_no_message_response ="SELECT count(s.id) as message_response from sms_sms s where 
						//~ $from_date 
             			//~ $to_date 
             			//~ and s.msg_type='Sent' and s.deleted=0 and s.msg_response!=''  and s.description='$description' Group By date_entered,description";
