...
class Columns_TargetCount extends Columns{
	...
	public function calculate($row, Row $class_row){
		global $db;
		$target = 0;
		if ($class_row->get_column('Unit_Name')->getValue()){
			$caller_id = $row['caller_id'];
			$query = "select * from users_cstm where id_c='$caller_id'";

			$result = $db->query($query);
			while($row = $db->fetchByAssoc($result)){
				$target = $row['targets_achieved_c'];
			}
		}
		$this->value = $target;
	}
...
class Columns_Fresh_Data_Given extends Columns{
	...
	public function calculate($row, Row $class_row){
		if($row and $class_row->get_column('Unit_Name')->getValue()){
			$this->value += 1;
		}
	}
}
...
class Report{
	...
	public function run($from_date, $to_date){
	    ...
	    $result = $db->query($query);
	    while($row = $db->fetchByAssoc($result)){
	    	$user_id = $row['caller_user_name'];
	    	if (!array_key_exists($user_id, $this->report)){
	    		$this->report[$user_id] = new Row();
	    	}

	    	foreach ($column_order as $column) {
	    		$this->report[$user_id]->get_column($column)->calculate($row, $this->report[$user_id]);
	    	}
	    }
...
class scrm_Custom_ReportsViewgenerate_agentlevel_report extends SugarView {
    ...
    function display()
	{
		...
		$report_name = "Agent Level Report";
...
HTML_Search;
		...
		<div id="mainData">
			...
			<tr  height="25">
			<th  $header_style><label>Agent Name</lable></th>
			<th  $header_style><label>Agent User Id</lable></th>
