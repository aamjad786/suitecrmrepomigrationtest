...
class Units{
	...
	public function Units(){
		$this->units = array('Call Center-Tata BSS', 'Call Center-Kenkei', 'Call Center-KServe');
	}
	...
	public function calculateSecurityGroup($cam_or_fos){
		...
		if($security_group_raw){
			foreach ($unit_array as $unit) {
				if (strpos($security_group_raw, $unit) !== false){
					return 'FOS';
				}
			}
			return 'Sales';
...
class Columns_Unit_Name extends Columns{
	...
	public function calculate($row, Row $class_row = NULL){
		...
		$caller_id = $row['caller_id'];
		$security_group_raw = SecurityGroupMap::getSecurityGroup($caller_id);
		$unit_array = $this->units->get_units();
...
class Columns_TargetCount extends Columns{
	...
	public function calculate($row, Row $class_row){
		global $db;
		$target = 0;
		$unit = $class_row->get_column('Unit_Name')->getValue();
		if (!$this->value){
			$query = "select * from users_cstm";

			$result = $db->query($query);
			while($row_user = $db->fetchByAssoc($result)){
				$security_group_raw = SecurityGroupMap::getSecurityGroup($row_user['id_c']);
				if (strpos($security_group_raw, $unit) !== false)
					$target += $row_user['targets_achieved_c'];
			}
			$this->value = $target;
		}
	}
...
class Columns_Total_Pickups extends Columns{
	...
	public function calculate($row, Row $class_row){
		...
		// $cam_or_fos = $row['cam_or_fos'];
		// if (!empty($cam_or_fos) and $cam_or_fos != ''){
		// 		$this->value += 0;
		// }
	}
}
...
class Columns_Percent_Achievement extends Columns{
	...
	public function calculate($row, Row $class_row){
		if ($class_row->get_column('TargetCount')->value != 0){
			$this->value = round(100*($class_row->get_column('Cases_Disbursed_FOS')->value/$class_row->get_column('TargetCount')->value), 2);
		}
	}
}
...
class Report{
	...
	public function run($from_date, $to_date){
		...
		$this->column_order_obj = new Column_Order;
		$column_order = $this->column_order_obj->get_column_order();
		$this->unit_name_obj = new Columns_Unit_Name();
		$units = new Units();
		$unit_array = $units->get_units();

		$this->report['Kenkei'] = new Row();
		$this->report['Kenkei']->get_column('Unit_Name')->setValue('Kenkei');
		$this->report['Tata BSS'] = new Row();
		$this->report['Tata BSS']->get_column('Unit_Name')->setValue('Tata BSS');
	    ...
	    $result = $db->query($query);
	    while($row = $db->fetchByAssoc($result)){
	    	$this->unit_name_obj->calculate($row);
	    	$unit_name = $this->unit_name_obj->getValue();
	    	if(in_array($unit_name, $unit_array)){
		    	if (!array_key_exists($unit_name, $this->report)){
		    		$this->report[$unit_name] = new Row();
		    	}

		    	foreach ($column_order as $column) {
		    		$this->report[$unit_name]->get_column($column)->calculate($row, $this->report[$unit_name]);
		    	}
		    }
	    }
...
class scrm_Custom_ReportsViewgenerate_projectlevel_report extends SugarView {
    ...
    function display()
	{
		...
		foreach($unit_array as $unit){
			foreach($report as $row){
				...
				if ($row->get_column('Unit_Name')->getValue() === $unit){
					$data .= '<tr height="25">';
					foreach ($column_order as $column){
		...
		$report_name = "Project Level Report";
...
HTML_Search;
		...
		<div id="mainData">
			...
			<tr  height="25">
			<th  $header_style><label>Unit Name</lable></th>
			<th  $header_style><label>Target Count</lable></th>
