...
require_once('modules/custom/Audit/Audit.php');
...
class Popup_Picker
{
	...
	{
		/*if( !ACLController::checkAccess($bean, 'create', true) )  {
...
EOHTML;
		...
		foreach($audit_list as $audit)
		{
			...
			{
				echo $before_value = $audit['before_value_text'];
            ...
            //Below if condition to convert date modified in to IST form UTC
			if($audit['field_name']=='Date Modified'){
				$before_value = date("Y-m-d H:i:s", strtotime("+5 hours, +30 minutes", strtotime($before_value)));
				$after_value = date("Y-m-d H:i:s", strtotime("+5 hours, +30 minutes", strtotime($after_value)));
				  }
