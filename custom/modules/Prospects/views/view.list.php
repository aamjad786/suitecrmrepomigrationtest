...
class ProspectsViewList extends SugarView{
    var $type ='list';
    var $lv;
    var $searchForm;
    var $use_old_search;
    ...
    function listViewPrepare(){
		//START Sorting targets in reverse chronological order by default (on any date created)
		 $_REQUEST['orderBy'] = strtoupper('target_date_assigned_c');             
	     $_REQUEST['sortOrder'] = 'desc'; 
	     //END Sorting targets in reverse chronological order 
...
HIDE;
        ...
        if($role[0] == 'Call Center Team Leader'){
			$current_user_id = $current_user->id;
			$query = "SELECT id, LTRIM( RTRIM( CONCAT( IFNULL( first_name, '' ) , ' ', IFNULL( last_name, '' ) ) ) ) AS name
					  FROM users
					  WHERE reports_to_id = '$current_user_id'
					  AND deleted =0";
			  
			$result = $db->query($query);
			$select = '<select id="mass_update_user_id"><option value="---"></option>';
			...
			echo $js=<<<EOF
				...
				$(document).ready(function(){
					...
					//Hide mass update standard fields
					$('select[name="optout_primary"]').parent().parent().hide();
					$('#mass_do_not_call').hide();
					$('#mass_do_not_call').parent().prev().hide();
					
					$('#mass_assigned_user_name').parent().append('<tr><td>$select</td></tr>');
					
					$('#mass_update_user_id').change(function(){
						
						var user_id = $('#mass_update_user_id').find('option:selected').val();
						var user_name = $('#mass_update_user_id').find('option:selected').text();
						
						$('#mass_assigned_user_id').val(user_id);
						$('#mass_assigned_user_name').val(user_name);
					});
				});
				</script>
EOF;
