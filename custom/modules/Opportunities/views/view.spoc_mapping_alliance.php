...
class OpportunitiesViewSpoc_mapping_alliance extends SugarView {
	...
	function printUserDetails($userBean){
		global $db;
		$query = "SELECT * FROM cluster_city_mapping_alliance where city='$userBean'";
	...
	function getCrmSpocDetails(){
    	global $db;
    	$users = array();
    	$query = "SELECT * FROM cluster_city_mapping_alliance";
    	$results = $db->query($query);
		$i=0;
	...
	<?php
        ...
        echo $html = <<<HTMLFORM
		<h1><center><b>Alliance SPOC Mapping Management</b></center></h1>
		<form action="index.php?module=Opportunities&action=spoc_mapping_alliance" method='post' autocomplete="off">
		<h2><b>Manual</b></h2>
		<table>
			<tr>
				<td>City:</td>
