...
//			_pp($UIDLs);_pp($cacheUIDLs);
	...
	function savePersonalEmailAccount($userId = '', $userName = '', $forceSave=true) {
		...
		if(isset($opts['serial']) && !empty($opts['serial'])) {
			...
			$stored_options['only_since'] = true;//$onlySince; To avoid a bug which resets the value to false on save
	...
	function handleCaseAssignment($email) {
		...
		if($caseId = $this->getCaseIdFromCaseNumber($email->name,$email->from_addr, $c)) {
			...
			if($email->description_html) {
				$description = ($email->description_html);
			}else{
...
eoq;
	...
	function getNewMessageIds() {
		...
		$storedOptions = unserialize(base64_decode($this->stored_options));
		fwrite($myfile, print_r($storedOptions,true));
		...
		if((true) || $storedOptions['only_since']) {// POP3 does not support Unseen flags
			...
			if(!isset($storedOptions['only_since_last']) && empty($storedOptions['only_since_last'])) {
				...
				$q = 'SELECT last_run FROM schedulers WHERE job = \'function::pollMonitoredInboxesAOP\'';
				$r = $this->db->query($q, true);
				$a = $this->db->fetchByAssoc($r);

				$date = date('r', strtotime($a['last_run']));
				fwrite($myfile,"\n In stored options date is not set, using scheduler date $date");
			} else {
				// echo "I am in else";
				$date = $storedOptions['only_since_last'];
				fwrite($myfile,"\nIn else found the date $date");
			}
			//return;
		...
		fwrite($myfile, '\n-----> getNewMessageIds() got '.count($ret).' new Messages');
