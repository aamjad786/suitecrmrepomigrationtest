...
function getASRejectionReasons($rejections,$app_id){
    ...
    if (!empty($rejection_ids)){
        foreach($rejection_ids as $reject_id){
        	$as_api_url 		= "http://192.168.11.186:3001/applications/1000282/get_rejection_reason?rejection_id=".$reject_id;
...
function getASAppStageDetails($app_id){
	...
	require_once 'custom/include/CurlReq.php';
    $cl 				= new CurlReq();
	$as_api_url 		= "http://192.168.11.186:3001/external_interfaces/get_application_status?application_id=".$app_id;
	    			...
	    			if(!empty($custom_status)){
	    				if($custom_status != $item->custom_status){
	    					...
	    					if($custom_status=="Loan disbursed"){
								...
								$url 		= getenv('SCRM_AS_API_BASE_URL')."http://192.168.11.186:3001/applications/$app_id/get_funded_amount";
