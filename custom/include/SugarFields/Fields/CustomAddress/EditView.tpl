...
<td>
<select name="{{$city}}" style="width: 250px" id="{{$city}}" title="{{$vardef.help}}" tabindex="{{$tabindex}}" {{if isset($displayParams.script)}}{{$displayParams.script}}{{/if}}>
{if isset($fields.{{$city}}.value) && $fields.{{$city}}.value != ''}
     {html_options options=$fields.{{$city}}.options selected=$fields.{{$city}}.value}
{else}
     {html_options options=$fields.{{$city}}.options selected=$fields.{{$city}}.default_value}
{/if}
</select>
</td>
